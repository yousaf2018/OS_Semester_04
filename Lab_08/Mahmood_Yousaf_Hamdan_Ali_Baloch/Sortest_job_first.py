# -*- coding: utf-8 -*-
"""
Created on Thu Mar 12 22:18:47 2020

@author: Mahmood Yousaf && Hamdan Ali Baloch
"""
...
def find_avg_time(process, n, burst_time, arrival_time):
    ...
    with open("results.txt","a") as file:
        file.write(f"Avergae waiting time in SJF for all processess is {total_wait_tim/n}\n")
        file.write(f"Turn around time in SJF for all processess is {total_turn_arn_time/n}\n")
        file.write(f"Avergae Respose time in SJF for all processess is {total_wait_tim/n}\n")
        file.close()
    print(f"Avergae waiting time in shortes job first for all processess is {total_wait_tim/n}\n")
    print(f"Turn around time in shortes job first for all processess is {total_turn_arn_time/n}\n")
    print(f"Avergae Respose time in shortes job firstfor all processess is {total_wait_tim/n}\n")
...
with open("jobs_10000.csv","r") as file:
    ...
    for row in reader_handler:
        ...
        i += 1
#Loop for sorting burst time with respect to arrival time 
    for i in range(0,10000):
        for u in range(0,10000):
            if burst_time[i] < burst_time[u]:
                ...
                process[u] = process[i]
                process[i] = tempo
                tempo = arrival_time[u]
                arrival_time[u] = arrival_time[i]
                arrival_time[i] = tempo
size=len(process) 
